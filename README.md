...
- **应用程序 → 办案工具集 → 办公助手**（`/Applications/办案工具集/办公助手.app`，推荐）：独立 arm64 应用，当前版本 2.3.2；可用 `macos/build_macos_app.sh` 重复构建
...
1. **资料完整性** — 委托人/对方/律师/案由或罪名/阶段/收费缺失即阻断；刑事案件不要求对方当事人；公益法律不要求案由；**案情摘要缺失仅提示，不拦截**
1a. **案由规范性**（2026-07-10 新增）— 除公益法律外，案由/罪名必须命中 OA 系统案由库（`GetCaseHeads` 全量 12 棵树约 2009 节点，含刑事罪名/民事案由 2011 版等，进程内缓存 12h）：自由填写文本**硬阻断**并给出相近规范案由建议；顶级分类节点硬阻断；非末级案由仅提示；含顿号罪名（如「拒不执行判决、裁定罪」）整串优先匹配，多案由顿号并列逐一校验；登记案由与字典关联案由不一致时提示。公益法律只核验 OA 系统大类，不要求案由或案由字典匹配
2. **利冲检索** — 对立方向命中须合伙人勾选「已人工复核」并填写结论（写入审批意见）
3. **OA 重复立案** — 同委托人+同对方+同案由时继续比较 OA 案件类型与当前程序阶段：一审/二审/再审/执行等不同阶段明确识别为关联案，不按重复立案处理；相同或无法识别阶段只转合伙人人工复核，不自动驳回；仅单方当事人重叠或案由不同只作信息提示，不阻断自动审批（2026-08-20 修正）
4. **低收费审查** — 委托收费 < ¥5,000 必须有具体收费说明（OA ChargeMemo），否则阻断；“前所/此前/一审已收费，本案不再收费”等明确区分历史收费与本案零收费的说明予以通过，避免把历史金额误认成本案收费（2026-08-19 修正）
